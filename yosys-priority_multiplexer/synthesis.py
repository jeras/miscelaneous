...
stepidx = 0
subprocess.Popen(f'mkdir -p reports', shell=True)

def report (name: str):
    global stepidx
    stepname = f"reports/{stepidx}_{name}"
    ys.run_pass(f"dump")
    ys.run_pass(f"stat")
...
# read design
ys.run_pass(f"read_verilog -icells -sv priority_multiplexer_wrapper.sv")

ys.run_pass(f"hierarchy -check -top priority_multiplexer_wrapper")
